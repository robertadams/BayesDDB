...
bdb_single_arm_cont <- function(
  ...
  # Alternatively could use `rlang::check_installed("rjags", "must be installed")` that provides an interactive prompt that allows the user to install rjags
  ...
  # !RH this is unnecessary since these packages are already in Imports
  ...
  for (i in 1:s) {
    e.m[i] <- mean(data1$Y[data1$Z == 1 & data1$stratum == i])
  }
  ...
  for (i in 1:s) {
    r_s1[i, ] <- mean_CI(data1$Y[data1$Z == 1 & data1$stratum == i], za)
  }
  ...
  if (plots) {
    ...
    fp1 <- ggplot2::ggplot(
      ...
      ggplot2::ggtitle('Mean estimate by Stratum and Overall') +
      ggplot2::theme_bw()
    df <- data.frame(
      x = data1$prd,
      Control = c(
    ...
    fp9 <- ggplot2::ggplot(
      ...
      ggplot2::xlab('') +
      ggplot2::ylab('Posterior Mean (95% CrI)') +
      ggplot2::ggtitle('Overall sample mean for ECA and SAT') +
      ggplot2::theme_bw()
    temp <- 'Stratum 1'
    if (s > 1) {
      for (i in 2:s) {
    ...
    fp10 <- ggplot2::ggplot(
      ...
      ggplot2::coord_flip() +
      ggplot2::ggtitle('Sample mean for SAT and ECA by stratum') +
      ggplot2::theme_bw()
    plots_list <- list(
      forest = fp1,
      ps_group = fp2,
