...
bdb_single_arm_bin <- function(
  ...
  PPP <- rep(0, s)
  for (i in 1:s) {
    PPP[i] <- mean(rbinom(N, n_s1[i], prb[i, ]) / n_s1[i] > e.m[i])
  }
  ...
  for (i in 1:s) {
    ...
    theta[i, ] <- rbeta(
      K,
      alpha + ne_s1[i] + ne_s0[i] * gamma[i] * gPPP[i],
      beta + (n_s1[i] - ne_s1[i]) + (n_s0[i] - ne_s0[i]) * gamma[i] * gPPP[i]
    )
  ...
  r_s1 <- t(rbind(
    ne_s1 / n_s1,
    qbeta(za / 2, ne_s1, n_s1 - ne_s1 + 1),
    qbeta(1 - za / 2, ne_s1 + 1, n_s1 - ne_s1)
  ))

  # Overall event rate from SAT and confidence interval from beta distribution (Clopper-Person interval)
  R_SAT <- c(
    ne_1 / n_1,
    qbeta(za / 2, ne_1, n_1 - ne_1 + 1),
    qbeta(1 - za / 2, ne_1 + 1, n_1 - ne_1)
  )
  ...
  if (plots) {
    ...
    fp1 <- ggplot2::ggplot(
      ...
      ggplot2::ggtitle('Event rate by Stratum and Overall') +
      ggplot2::theme_bw()
    df <- data.frame(
      x = data1$prd,
      Control = c(
    ...
    fp9 <- ggplot2::ggplot(
      ...
      ggplot2::xlab('') +
      ggplot2::ylab('Posterior Mean (95% CrI)') +
      ggplot2::ggtitle('Overall event rate for ECA and SAT') +
      ggplot2::theme_bw()
    temp <- 'Stratum 1'
    if (s > 1) {
      for (i in 2:s) {
    ...
    fp10 <- ggplot2::ggplot(
      ...
      ggplot2::xlab('') +
      ggplot2::ylab('Event rate [95% CrI]') +
      ggplot2::coord_flip() +
      ggplot2::ggtitle('Event rate for ECA and SAT by stratum') +
      ggplot2::theme_bw()
    plots_list <- list(
      forest = fp1,
      ps_group = fp2,
