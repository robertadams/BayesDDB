...
#' }
#' @export
bdb_two_arms_bin <- function(Y, Z, TRT, X, A, s = 3, a = 1, alpha = 0.5, beta = 0.5,
  ...
  for (i in 1:s){
    ...
    # Controls
    # Sampling from beta-binomial distribution with parameters shape parameters alpha and beta
    thetaC[i,]<-rbeta(K, alpha + ne_s10[i] + ne_s0[i]*gamma[i]*gPPP[i], beta + (n_s10[i]-ne_s10[i]) + (n_s0[i]-ne_s0[i])*gamma[i]*gPPP[i])
    # Mean rate by stratum
  ...
  diff_n<-rbeta(K, alpha+ne_11, beta+n_11-ne_11) - rbeta(K, alpha+ne_10, beta+n_10-ne_10)
  ...
  if (plots == TRUE) {
    ...
    fp9 <- ggplot2::ggplot(data = df, ggplot2::aes(x = label, y = mean, ymin = lower, ymax = upper, fill = group)) +
      ...
      ggplot2::coord_flip() + ggplot2::xlab('') + ggplot2::ylab('Posterior Mean (95% CrI)') + ggplot2::ggtitle('Overall event rate for ECA and ICA') + ggplot2::theme_bw()
    temp <- 'Stratum 1'
    if (s > 1) { for (i in 2:s) { temp <- c(temp, paste('Stratum', i)) } }
    temp <- c(temp, temp)
    label <- temp
    ...
    fp10 <- ggplot2::ggplot(data = df, ggplot2::aes(x = label, y = rate, ymin = lower, ymax = upper, fill = group)) +
      ggplot2::geom_linerange(linewidth = 5, position = ggplot2::position_dodge(width = 0.25), colour = "lightgrey") +
      ggplot2::geom_point(size = 3, shape = 21, stroke = 0.25, position = ggplot2::position_dodge(width = 0.25)) +
      ggplot2::scale_x_discrete(name = " ") + ggplot2::scale_y_continuous(name = "Event rate [95% CrI]") + ggplot2::coord_flip() +
      ggplot2::ggtitle('Event rate for ECA and ICA by stratum') + ggplot2::theme_bw()
  ...
  output <- list(n_s1 = n_s1, n_s0 = n_s0, n_s11 = n_s11, n_s10 = n_s10,
                 ...
                 r_ica = R_ICA, r_eca = R_ECA, theta_n = theta_n,
