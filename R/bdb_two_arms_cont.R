...
bdb_two_arms_cont <- function(
  ...
  if (!requireNamespace("rjags", quietly = TRUE)) {
    stop(
      "Package 'rjags' is required. Install it and ensure JAGS is installed.",
      call. = FALSE
    )
  }
  ...
  for (i in 1:s) {
    e.m[i] <- mean(data1$Y[data1$Z == 1 & data1$TRT == 0 & data1$stratum == i])
  }
  ...
  for (i in 1:s) {
    ...
    if (N1_s == 0) {
      N1_s = 1
    } # otherwise the model does not run
    dataList <- list(
      "n" = n0_s,
      "y" = y0_s,
      "N1" = N1_s,
  ...
  for (i in 1:s) {
    r_s0[i, ] <- mean_CI(data1$Y[data1$Z == 0 & data1$stratum == i], za)
  }
  # Mean and 95% CI for ICA by stratum
  r_s10 <- matrix(data = NA, nrow = s, ncol = 3)
  for (i in 1:s) {
    r_s10[i, ] <- mean_CI(
      data1$Y[data1$Z == 1 & data1$TRT == 0 & data1$stratum == i],
      za
    )
  }
  ...
  for (i in 1:s) {
    ...
    pre_ini <- 1 /
      var(data1$Y[data1$Z == 1 & data1$TRT == 1 & data1$stratum == i])
    ...
    gamma_ <- data1$Z[data1$stratum == i] +
      (data1$Z[data1$stratum == i] == 0) * gamma[i] * gPPP[i]
    zeros_ <- rep(0, n_)
    data_CONT <- list(
      n = n_,
      y = y_,
    ...
    pre_ <- 1 / var(y_)
    inits_CONT <- function() {
      list(mu = mu_, pre = pre_)
    }
    # Fit model
    fit_cont_s <- rjags::jags.model(
      file = modelfile,
      data = data_CONT,
      inits = inits_CONT,
      n.chains = 1
    ...
    fit_cont_s <- rjags::coda.samples(
      fit_cont_s,
      variable.names = params_CONT,
      n.iter = n.iter,
      thin = 1
    )
  ...
  if (plots) {
    ...
    fp9 <- ggplot2::ggplot(
      ...
      ggplot2::xlab('') +
      ggplot2::ylab('Posterior Mean (95% CrI)') +
      ggplot2::ggtitle('Overall sample mean for ECA and ICA') +
      ggplot2::theme_bw()
    temp <- 'Stratum 1'
    if (s > 1) {
      for (i in 2:s) {
    ...
    fp10 <- ggplot2::ggplot(
      ...
      ggplot2::scale_y_continuous(name = "Sample mean [95% CI]") +
      ggplot2::coord_flip() +
      ggplot2::ggtitle('Sample mean for ECA and ICA by stratum') +
      ggplot2::theme_bw()
    plots_list <- list(
      forest = fp1,
      ps_group = fp2,
