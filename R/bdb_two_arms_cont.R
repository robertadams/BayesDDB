#' Bayesian Dynamic Borrowing for Two-Arm Trial with Continuous Outcome
#'
#' Applies the Bayesian Dynamic Borrowing double-adjustment method when the control arm
#' of a randomized trial is augmented with external data (continuous outcome). Requires JAGS.
#'
...
#'
#' @return A list with \code{N0_t}, \code{n_s0}, \code{n_s1}, \code{n_s11}, \code{n_s10}, \code{r_s0}, \code{r_s10},
#'   \code{r_ica}, \code{r_eca}, \code{A_eff}, \code{v}, \code{r}, \code{gamma}, \code{PPP}, \code{gPPP},
#'   \code{mean.theta}, \code{var.theta}, \code{CrI}, \code{Width}, \code{theta_s}, \code{theta_o}, \code{theta} (same as \code{theta_o}),
...
#'
#' @seealso \code{\link{bdb_single_arm_bin}}, \code{\link{bdb_single_arm_cont}}, \code{\link{bdb_two_arms_bin}}, \code{\link{elastic_function}}
#'
#' @examples
#' \dontrun{
...
#' }
#' @export
bdb_two_arms_cont <- function(Y, Z, TRT, X, A, s = 3, a = 1, za = 0.05, n_iter = 5000,
  ...
  mylogit<-glm(Z ~ .-Y - TRT, data = data, family = stats::binomial())
  ...
  e.m<-rep(0,s) # observed mean in internal control by stratum
  ...
  for(i in 1:s){
    ...
    N1_s<-sum(data1$Z==1 & data1$TRT==0 & data1$stratum==i)
  ...
  n_s11<-rep(0,s)
  for (i in 1:s){ n_s11[i]<-sum(data1$Z==1 & data1$TRT==1 & data1$stratum==i) }
  
  n_s10<-rep(0,s)
  for (i in 1:s){ n_s10[i]<-sum(data1$Z==1 & data1$TRT==0 & data1$stratum==i) }
  
  # Discount parameter
  gamma<-rep(0,s)
  for (i in 1:s){ gamma[i]<-min(1,A*r[i]/n_s0[i]) }
  
  # Mean and95% CI for ICA
  R_ICA<-mean_CI(data1$Y[data1$Z==1 & data1$TRT==0], za)
  ...
  # Mean and 95% CI for ICA by stratum
  r_s10<-matrix(data=NA, nrow=s, ncol=3)
  for(i in 1:s){r_s10[i,]<-mean_CI(data1$Y[data1$Z==1 & data1$TRT==0 & data1$stratum==i], za)}
  ...
  # Initialize mu_TREAT, mu_CONT, and theta objects
  mu_TREAT<-matrix(data=rep(0, s*n.iter), nrow=s, ncol=n.iter)
  mu_CONT<-matrix(data=rep(0, s*n.iter), nrow=s, ncol=n.iter)
  ...
  for (i in 1:s){ 
    ...
    # Initial values
    mu_ini<-mean(data1$Y[data1$Z==1 & data1$TRT==1 & data1$stratum==i])
    pre_ini<-1/var(data1$Y[data1$Z==1 & data1$TRT==1 & data1$stratum==i])
    inits<-list(mu=mu_ini, pre=pre_ini)
    # Data list as input to JAGS
    n11_s<-sum(data1$Z==1 & data1$TRT==1 & data1$stratum==i)
    y11_s<-data1$Y[data1$Z==1 & data1$TRT==1 & data1$stratum==i]
    dataList<-list("n"=n11_s,"y"=y11_s, "alpha"=alpha, "beta"=beta, "tau"=tau) 
    # Run MCMC by JAGS
    fit_treat_s <- rjags::jags.model(model_TREAT, data=dataList, inits=inits, n.chains=1, n.adapt=10000) 
    # Draw MCMC samples
    params_TREAT<-c("mu","pre") # Parameters to be monitored in MCMC
    out <- rjags::coda.samples(fit_treat_s, variable.names = params_TREAT, n.iter=n.iter)
    out<-do.call(rbind.data.frame, out)
    # Posterior samples
    mu_TREAT[i,]<-out$mu
    ...
    # Data list
    n_<-sum(data1$TRT==0 & data1$stratum==i)
    y_<-data1$Y[data1$TRT==0 & data1$stratum==i]
    ...
    # Fit model 
    fit_cont_s <- rjags::jags.model(file = modelfile, data = data_CONT, inits = inits_CONT, n.chains = 1)
  ...
  if (plots == TRUE) {
    ...
    fp1 <- ggplot2::ggplot(data = df, ggplot2::aes(x = label, y = mean, ymin = lower, ymax = upper)) +
      ggplot2::geom_pointrange(linewidth = 1) + ggplot2::geom_hline(yintercept = 0, lty = 2) + ggplot2::coord_flip() +
      ggplot2::geom_text(x = 1.2, y = theta_o[1], label = paste(round(theta_o[1], 2), ' [', round(theta_o[2], 2), ', ', round(theta_o[3], 2), ']', sep = '')) +
      ggplot2::xlab('') + ggplot2::ylab('Posterior Mean (95% CrI)') + ggplot2::ggtitle('Treatment Effect by Stratum and Overall') + ggplot2::theme_bw()
    ...
    fp9 <- ggplot2::ggplot(data = df, ggplot2::aes(x = label, y = mean, ymin = lower, ymax = upper, fill = group)) +
      ...
      ggplot2::coord_flip() + ggplot2::xlab('') + ggplot2::ylab('Posterior Mean (95% CrI)') + ggplot2::ggtitle('Overall sample mean for ECA and ICA') + ggplot2::theme_bw()
    temp <- 'Stratum 1'
    if (s > 1) { for (i in 2:s) { temp <- c(temp, paste('Stratum', i)) } }
    temp <- c(temp, temp)
    label <- temp
    ...
    fp10 <- ggplot2::ggplot(data = df, ggplot2::aes(x = label, y = rate, ymin = lower, ymax = upper, fill = group)) +
      ...
      ggplot2::ggtitle('Sample mean for ECA and ICA by stratum') + ggplot2::theme_bw()
    plots_list <- list(forest = fp1, ps_group = fp2, ps_stratum = fp3, mcmc = fp7, overall_rates = fp9, rates_by_stratum = fp10)
  }
  output <- list(N0_t = n0, n_s0 = n_s0, n_s1 = n_s1, n_s11 = n_s11, n_s10 = n_s10, r_s0 = r_s0, r_s10 = r_s10,
