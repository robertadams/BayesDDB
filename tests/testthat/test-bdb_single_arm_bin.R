test_that("bdb_single_arm_bin returns list with expected elements", {
  ...
  out <- bdb_single_arm_bin(Y = Y, Z = Z, X = X, A = 10, s = 2, a = 1, K = 500, show_plots = FALSE)
...
test_that("bdb_single_arm_bin with double_adj = FALSE runs", {
  ...
  out <- bdb_single_arm_bin(Y = Y, Z = Z, X = X, A = 5, s = 2, double_adj = FALSE, K = 300, show_plots = FALSE)
  expect_type(out, "list")
  expect_true("theta_o" %in% names(out))
})
